...
func NetProbe(proxy *Proxy, address string, timeout int) error {
	...
	for tries := timeout; tries > 0; tries-- {
		...
		if err == nil {
			...
			// Write fails with WSAENOBUFS: "An operation on a socket could not be performed because the system lacked sufficient buffer space or because a fetchLock was full"
